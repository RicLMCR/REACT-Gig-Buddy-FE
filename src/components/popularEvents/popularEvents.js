...
export const PopularEvents = ({
  ...
  return (
    <div className="pop-container">
      ...
      <h1 className="popular-events-mancester-header">
        Live Events in Manchester
      </h1>
      ...
      <div className="pop-events">
        {
          trendingEvents.map((item, index) => {
            return (
              <div key={index}>
                <img
                  className="pop-big-images"
                  src={item.xlargeimageurl}
                  alt=""
                />
                ...
                <div className="pop-events-big-image-description">
                  <div>
                    ...
                    <div className="big-image-event-name">
                      <p>{item.eventname}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => onSearch(item.eventname, item)}
                    className="pop-events-btn"
                  >
        ...
        <div className="small-image-list-flex">
          {trendingEvents
            .filter(
              (item, index) =>
                index < `${listNumbers.larger}` && index > `${listNumbers.less}`
            )
            .map((item, index) => {
              return (
                <div className="pop-events-small-image" key={index}>
                  <img
                    className="pop-events-small-image"
                    src={item.largeimageurl}
                    alt=" image"
                  />
                  <button
                    onClick={() => onSearch(item.eventname, item)}
                    className="pop-events-btn-small"
                  >
                    <AiOutlineArrowRight size={20} color="white" />
                  </button>
      ...
      <div className="event-buttons">
        <button onClick={() => pageChange(4, 0, 0)} className="next-btn">
          1
        </button>
        <button onClick={() => pageChange(8, 4, 4)} className="next-btn">
          2
        </button>
        <button onClick={() => pageChange(12, 8, 8)} className="next-btn">
          3
        </button>
        <button onClick={() => pageChange(16, 12, 12)} className="next-btn">
          4
        </button>
        <button onClick={() => pageChange(20, 16, 16)} className="next-btn">
          5
        </button>
      </div>
    </div>
